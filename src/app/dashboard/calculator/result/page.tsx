...
function CostRow({
  ...
  return (
    <div className={cn(indent && "pl-4")}>
      ...
      {detail && <p className="tabular-nums mt-0.5 text-[11px] text-zinc-400">{detail}</p>}
    </div>
  );
}
...
// ── PriceCard ────────────────────────────────────────────
function PriceCard({
  label, floorRange, marketPrice, minPrice, maxPrice, count, baseCost, myInvestment,
}: {
  label: string; floorRange: string; marketPrice: number;
  minPrice: number; maxPrice: number; count: number;
  baseCost: number; myInvestment: number;
}) {
  if (marketPrice === 0) {
    return (
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="space-y-0.5">
            <div className="flex items-center gap-1.5">
              ...
              <span className="tabular-nums text-[12px] text-zinc-600">{formatKoreanWon(minPrice)}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-zinc-500 w-7 shrink-0 font-medium">평균</span>
              <span className="tabular-nums text-[14px] font-bold text-zinc-900">{formatKoreanWon(marketPrice)}</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] text-zinc-400 w-7 shrink-0">고가</span>
              <span className="tabular-nums text-[12px] text-zinc-600">{formatKoreanWon(maxPrice)}</span>
            </div>
          </div>
        </div>
...
export default async function ResultPage({ searchParams }: Props) {
  ...
  const priceAnalysis = row.price_analysis as {
    ...
    dataCount: number; period: string;
  ...
  // 출구전략 기준 매도가: mid → 없으면 low → 없으면 high
  const exitSalePrice =
    (priceAnalysis?.mid ?? 0) > 0 ? (priceAnalysis!.mid) :
  ...
  return (
    <div className="mx-auto max-w-[640px] py-10">
        ...
        {hasLoan && (
          <div className="mt-4 flex items-center gap-6 rounded-xl bg-zinc-50 px-4 py-3">
            <div>
              ...
              <p className="tabular-nums mt-0.5 text-sm font-semibold text-zinc-700">{formatKoreanWon(loanAmount)}</p>
            </div>
            <div className="h-8 w-px bg-zinc-200" />
            <div>
              <p className="text-[11px] text-zinc-400">실투자금</p>
              <p className="tabular-nums mt-0.5 text-sm font-bold text-zinc-900">{formatKoreanWon(myInvestment)}</p>
            </div>
            <div className="h-8 w-px bg-zinc-200" />
            <div>
      ...
      <div className="mb-4 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-baseline justify-between">
          ...
          {hasPrice && priceAnalysis.period && (
            <p className="text-xs text-zinc-400">{priceAnalysis.period} · {priceAnalysis.dataCount}건</p>
          )}
        </div>
        {hasPrice ? (
          <>
            <div className="flex flex-col gap-2">
              {(priceAnalysis.lowCount ?? 0) > 0 && (
                <PriceCard label="저층" floorRange={lowRange}
                  marketPrice={priceAnalysis.low} minPrice={priceAnalysis.lowMin ?? 0}
                  maxPrice={priceAnalysis.lowMax ?? 0} count={priceAnalysis.lowCount ?? 0}
                  baseCost={baseCost} myInvestment={myInvestment} />
              )}
              {(priceAnalysis.midCount ?? 0) > 0 && (
                <PriceCard label="중층" floorRange={midRange}
                  marketPrice={priceAnalysis.mid} minPrice={priceAnalysis.midMin ?? 0}
                  maxPrice={priceAnalysis.midMax ?? 0} count={priceAnalysis.midCount ?? 0}
                  baseCost={baseCost} myInvestment={myInvestment} />
              )}
              {(priceAnalysis.highCount ?? 0) > 0 && (
                <PriceCard label="고층" floorRange={highRange}
                  marketPrice={priceAnalysis.high} minPrice={priceAnalysis.highMin ?? 0}
                  maxPrice={priceAnalysis.highMax ?? 0} count={priceAnalysis.highCount ?? 0}
                  baseCost={baseCost} myInvestment={myInvestment} />
              )}
            </div>
            <p className="mt-3 text-[11px] text-zinc-400">
              * 수익 = 매도가 − 취득가 − 중개수수료 (대출이자·중도상환수수료 별도)
              {hasLoan && " · 투자금 대비 수익률은 실투자금 기준"}
            </p>
          </>
        ) : (
      ...
      {/* 출구전략별 수익 비교 */}
      {hasPrice && exitSalePrice > 0 && (
        <ExitStrategyTable
          input={{
            baseCost,
            myInvestment,
            salePrice: exitSalePrice,
            inputHoldMonths: holdMonths,
          }}
          hasLoan={hasLoan}
        />
      )}
        ...
        <div className="space-y-2.5">
          ...
          <div className="border-t border-zinc-100 pt-2.5">
            ...
            <div className="space-y-2">
              ...
              <CostRow label="명도비용(평당)" value={evictionCost}               indent
                detail={evictionPerPyeong > 0 ? `${areaPyeong}평 × ${evictionPerPyeong}만원/평` : undefined} />
          ...
          {(loanInterest > 0 || prepaymentPenalty > 0) && (
            <div className="rounded-lg bg-zinc-50 px-3 py-2.5">
              <p className="text-xs text-zinc-500">
                수익 계산 기준 취득가{" "}
                <span className="tabular-nums font-semibold text-zinc-700">{formatManwon(baseCost)}</span>
